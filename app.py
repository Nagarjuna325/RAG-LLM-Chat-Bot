...
# Load API keys from environment variables (do not assign None)
openai_env_key = os.getenv("OPENAI_API_KEY")
if openai_env_key:
    os.environ['OPENAI_API_KEY'] = openai_env_key

groq_env_key = os.getenv("GROQ_API_KEY")
if groq_env_key:
    os.environ['GROQ_API_KEY'] = groq_env_key
    ...
    with st.spinner("Creating vector database - please wait..."):
        try:
            ...
            if not os.getenv("OPENAI_API_KEY"):
                st.error("OPENAI_API_KEY not set. Provide it in your environment or a .env file.")
                return False
...
def query_documents(user_prompt):
    ...
    try:
        ...
        try:
            ...
            if not groq_api_key:
                st.error("GROQ_API_KEY not set. Please set it in your environment or .env file to connect to Groq.")
            else:
